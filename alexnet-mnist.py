...
# Write for edited code, import the edited version of tensorFI
...
with tf.Session() as sess:
    if(train):
        ...
        saver.restore(sess, tf.train.latest_checkpoint('./alexnet-checkpoint'))
        print(sys.path)
        fi = ti.TensorFI(sess,configFileName='./confFiles/eb/default-3eb.yaml', logLevel = 10, name = "alexnet", disableInjections=False)
        print "Testing Accuracy:", sess.run(accuracy, feed_dict={x: mnist.test.images[:256], y: mnist.test.labels[:256], keep_prob: 1.})
        ...
        correct = sess.run(correct_pred, feed_dict={x: X_test, y: y_test, keep_prob: 1.})
